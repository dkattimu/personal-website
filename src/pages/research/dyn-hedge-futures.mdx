...
\Delta H^\xi_F(t_1, \omega)& := H^\xi_F(t_1, \omega)- H^\xi_F(t_1^-, \omega)\\
 ...
 &\vdots\\
H^\xi_F(t_k^-, \omega) &=V(t_{k-1}, \omega)e^{r^\xi(t_{k-1}, \omega)\Delta t_{k-1}}+ \Delta^V_F(t_{k-1}, \omega)\underbrace{\Big(F_S(t_k, \omega)-F_S(t_{k-1}, \omega)\Big)}_{\Delta F_S(t_{k-1})}\\
